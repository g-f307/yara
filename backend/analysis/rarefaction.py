...
Retorna dados estruturados (sem matplotlib — visualização via Plotly no frontend).

Migrado de: actions/utils/rarefaction_analyzer.py
...
class RarefactionAnalyzer:
    ...
    def calculate_saturation(self, sample_id: str) -> float:
        ...
        final_change = abs(float(values[-1]) - float(values[-2])) / float(values[-1]) if float(values[-1]) > 0 else 0
    ...
    def recommend_sampling_depth(self, min_samples_retained: float = 0.8) -> Dict:
        ...
        saturations = {}
        for depth in self.depths:
            avg_saturation = float(self.data[depth].mean())
        ...
        for depth in self.depths:
            ...
            valid_samples = int(self.data[depth].notna().sum())
        ...
        if best_depth:
            ...
            recommendations['samples_discarded'] = [str(s) for s in discarded]
    ...
    def get_summary_stats(self) -> Dict:
        ...
        if saturations:
            ...
            stats['saturation']['samples_saturated'] = int(sum(1 for s in saturations if s > 0.95))
    ...
    def get_curve_data(self, max_samples: int = 20) -> Dict:
        """
        Retorna dados das curvas para renderização no frontend (Plotly).
        Substitui o antigo plot_rarefaction_curves() que usava matplotlib.
        
        Args:
            max_samples: Número máximo de amostras
            
        Returns:
            Dicionário com dados das curvas
        """
        ...
        for sample_id in samples_to_plot:
            ...
            curves.append({
                ...
                'depths': depths_numeric,
                'values': [float(v) for v in sample_data.values]
            })
        
        return {
            'curves': curves,
            'x_label': 'Profundidade de Sequenciamento',
            'y_label': 'Número de Features Observadas',
